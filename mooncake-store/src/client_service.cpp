...
tl::expected<void, ErrorCode> Client::warmup(
    ...
    LOG(INFO) << "warmup: pre-establishing connections to " << segments.size()
              << " segment(s) for protocol '" << protocol_ << "'";
    ...
    auto submit_and_wait = [&](Transport::TransferRequest::OpCode op,
        ...
        request.opcode = op;
        request.source =
            buf_handle->ptr();  // registered buffer carries r/w data
        ...
        if (!status.ok()) {
            LOG(WARNING) << "warmup: submitTransfer("
                         << (op == Transport::TransferRequest::WRITE ? "W"
                                                                     : "R")
        ...
        for (int poll = 0; poll < 1000; ++poll) {
            ...
            if (!s.ok()) break;
            if (ts.s == Transport::COMPLETED || ts.s == Transport::FAILED ||
    ...
    for (const auto& segment_name : segments) {
        ...
        if (target_id == (SegmentHandle)-1) {
            LOG(WARNING) << "warmup: cannot open segment '" << segment_name
                         << "'";
            continue;
        }
        ...
        bool ok = submit_and_wait(Transport::TransferRequest::WRITE, target_id,
                                  segment_name);
        ok = submit_and_wait(Transport::TransferRequest::READ, target_id,
                             segment_name) &&
             ok;
        if (ok) ++success_count;
    }
...
tl::expected<void, ErrorCode> Client::BatchPushOffloadObject(
    const std::string& requester_te_addr, const std::vector<std::string>& keys,
