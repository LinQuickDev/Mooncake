    ...
    -> tl::expected<void, ErrorCode> {
    {
                        ...
                        [&](Replica& replica) {
                            ...
                            for (auto& restore : restores) {
                                if (descriptor.transport_endpoint_ ==
                                    ...
                                    // UB NUMA segments can share an endpoint.
                            ...
                            if (match != nullptr) {
                                ...
                                restored_objects.try_emplace(
                                    &metadata,
                                    metadata.HasReplica(
                                        [this](const Replica& candidate) {
                                            return candidate
                                                       .is_memory_replica() &&
                                                   IsReplicaReadable(candidate);
                                        }));
                            }
                        });
                }
        ...
        for (const auto& [metadata, was_readable] : restored_objects) {
            ...
            if (!was_readable &&
                ...
                ApplySoftPinEvaluation(*metadata,
                                       metadata->RefreshRecoveredSoftPin(
                                           default_kv_soft_pin_ttl_));
            }
        }
...
auto MasterService::RemoveObjectHeartbeat(const UUID& client_id)
    ...
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    return local_ssd_manager_.PeekRemoves(client_id);
}
...
void MasterService::EnqueueRemoveTasks(const std::vector<UUID>& holder_ids,
    ...
    for (const auto& holder_id : holder_ids) {
        (void)local_ssd_manager_.EnqueueRemove(holder_id, task);
    }
}
...
auto MasterService::AckRemoveObjectHeartbeat(
    ...
    -> tl::expected<void, ErrorCode> {
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    const auto error = local_ssd_manager_.AckRemoves(client_id, tasks);
            ...
            if (accessor.Exists()) {
                ...
                if (!segment_mounted) {
                    ...
                    refused_unmounted = true;
                               ...
                               [&client_id](const Replica& existing) {
                                   return existing.is_local_disk_replica() &&
                                          existing.get_local_disk_client_id() ==
                                              client_id;
                               })) {
                    std::vector<Replica> replicas;
                    replicas.emplace_back(std::move(replica));
                    ...
                    obj_metadata.VisitReplicas(
                        ...
                        [&metadata](Replica& existing) {
                            existing.update_local_disk_location(
...
bool MasterService::CancelQueuedOffloadTask(TenantState& tenant_state,
    ...
    for (const auto& task : task_it->second) {
        mirror_clients.push_back(task.source_client_id);
    }
