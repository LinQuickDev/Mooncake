    ...
    auto log_breakdown = [&](const char* fallback_status,
        ...
        const auto total_us =
            std::chrono::duration_cast<std::chrono::microseconds>(
                ...
                .count();
        const char* status = read_stats.status == "ok"
                                 ? fallback_status
                                 : read_stats.status.c_str();
        const ErrorCode error = read_stats.error_code == ErrorCode::OK
                                    ? fallback_error
...
tl::expected<void, ErrorCode> FileStorage::MarkRemoved(const std::string& key) {
    ...
    // replica here. We mark each as a tombstone so GC can reclaim SSD space.
    {
        ...
        if (remove_result) {
            ...
            if (all_marked && !remove_result.value().empty()) {
                ...
                if (!ack_result) {
                    LOG(ERROR)
                        << "Failed to ACK remove tasks: " << ack_result.error();
                }
...
bool FileStorage::ReleaseBuffer(uint64_t batch_id) {
    ...
    if (breakdown_log) {
        MC_LOG(INFO) << "storage_release_breakdown batch_id=" << batch_id
                     << " total_us=" << total_us << " found=" << (found ? 1 : 0)
