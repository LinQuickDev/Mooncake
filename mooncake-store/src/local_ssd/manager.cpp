...
ErrorCode LocalSsdManager::EnqueueRemove(const UUID& client_id,
                                         const RemoveTaskItem& task) {
    auto client = FindClient(client_id);
    if (!client) return ErrorCode::SEGMENT_NOT_FOUND;
    auto& mailbox = client->record->mailbox;
    MutexLocker lock(&mailbox.mutex_);
    auto& pending = mailbox.pending_removes_;
    if (std::find(pending.begin(), pending.end(), task) == pending.end()) {
...
LocalSsdManager::PeekRemoves(const UUID& client_id) const {
    auto client = FindClient(client_id);
    if (!client) return tl::make_unexpected(ErrorCode::SEGMENT_NOT_FOUND);
    auto& mailbox = client->record->mailbox;
    MutexLocker lock(&mailbox.mutex_);
    return mailbox.pending_removes_;
}

ErrorCode LocalSsdManager::AckRemoves(
    const UUID& client_id, const std::vector<RemoveTaskItem>& tasks) {
    auto client = FindClient(client_id);
    if (!client) return ErrorCode::SEGMENT_NOT_FOUND;
    auto& mailbox = client->record->mailbox;
    MutexLocker lock(&mailbox.mutex_);
    std::erase_if(mailbox.pending_removes_, [&](const RemoveTaskItem& task) {
