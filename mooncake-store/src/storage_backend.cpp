...
tl::expected<void, ErrorCode> BucketStorageBackend::BatchLoad(
    ...
    pt_plan.End(0);
    if (stats) {
        stats->plan_us = std::chrono::duration_cast<std::chrono::microseconds>(
                             std::chrono::steady_clock::now() - plan_start)
                             .count();
    }
    ...
    for (auto& [bucket_id, read_plans] : bucket_read_plans) {
        ...
        if (uring_file != nullptr) {
            ...
            pt_uring.End(batch_read_result ? 0 : -1);
            if (stats) {
                stats->disk_read_us +=
                    std::chrono::duration_cast<std::chrono::microseconds>(
                        std::chrono::steady_clock::now() - disk_start)
                        .count();
                stats->io_mode = "io_uring_batch";
            }
...
bool BucketStorageBackend::CompactBuckets(
    ...
    std::unordered_map<int64_t, uint64_t> old_bucket_generations;
    {
        ...
        for (int64_t bid : bucket_ids) {
            ...
            int64_t ts =
                bucket->last_access_ns_.load(std::memory_order_relaxed);
            ...
            for (size_t i = 0; i < bucket->keys.size(); ++i) {
                ...
                if (map_it != object_bucket_map_.end() &&
                    ...
                    live_keys_info.push_back({key, bid, bucket->metadatas[i]});
                }
            }
        }
    }
    ...
    for (const auto& info : live_keys_info) {
        int64_t key_total = info.meta.key_size + info.meta.data_size;
    ...
    if (!group_full && !space_pressure) {
        ...
        LOG(INFO) << "[GC] CompactBuckets deferred: group_count=" << group_count
                  << " group_size=" << group_size << " bucket_keys_limit="
                  << bucket_backend_config_.bucket_keys_limit
                  << " bucket_size_limit="
                  << bucket_backend_config_.bucket_size_limit
    ...
    std::unordered_map<int64_t, std::vector<const GroupedKey*>> keys_by_bucket;
    ...
    std::set<int64_t> old_bucket_ids_set(bucket_ids.begin(), bucket_ids.end());
    ...
    std::vector<int64_t> buckets_to_delete;
    {
        ...
        new_bucket->last_access_ns_.store(new_last_access_ns,
                                          std::memory_order_relaxed);
        ...
        for (int64_t bid : buckets_to_delete) {
            auto it = buckets_.find(bid);
            if (it != buckets_.end()) {
                total_size_ -= it->second->data_size + it->second->meta_size;
                buckets_.erase(it);
                int64_t ts = old_buckets[bid].second;
                lru_index_.erase({ts, bid});
            }
...
void BucketStorageBackend::WaitForInflightReads(
    ...
    while (bucket->inflight_reads_.load(std::memory_order_acquire) > 0) {
        if (++spin_count > kMaxSpinIterations) {
            std::this_thread::yield();
            spin_count = 0;
            if (std::chrono::steady_clock::now() - wait_start > kMaxWaitTime) {
    ...
    while (gc_running_.load(std::memory_order_acquire)) {
        ...
        bool space_pressure = false;
        {
            ...
            if (bucket_backend_config_.max_total_size > 0) {
                double used_ratio =
                    ...
                    static_cast<double>(bucket_backend_config_.max_total_size);
        ...
        std::vector<int64_t> candidates;
        {
            ...
            for (auto it = buckets_.begin();
                 ...
                 ++it) {
                int64_t deleted =
                    it->second->deleted_bytes_.load(std::memory_order_relaxed);
                if (deleted <= 0) continue;
                if (it->second->compacting_.load(std::memory_order_relaxed))
                    continue;
        ...
        if (!candidates.empty()) {
            if (bucket_backend_config_.gc_merge_enable &&
                ...
                if (CompactBuckets(candidates, space_pressure)) {
                    LOG(INFO)
                        << "[GC] merged " << candidates.size() << " bucket(s)";
                } else {
                ...
                for (int64_t bid : candidates) {
                    if (CompactBuckets({bid}, space_pressure)) {
                        ...
                        LOG(WARNING) << "[GC] CompactBuckets failed for bucket "
...
void OffsetAllocatorStorageBackend::EvictToMakeRoom(
    ...
    while (n < cfg_.max_evict_per_offload) {
        ...
        auto& shard = shards_[shard_idx];
        {
            ...
            // eviction only becomes final when the master accepts the
            // notification; if it fails, RestorePreparedEviction rolls
            // the entry back and a tombstone would wrongly delete the
            // still-valid key on recovery.  Tombstones are recorded at
            // commit time in NotifyAndCommitPreparedEviction.
            metadata_dirty_.store(true, std::memory_order_relaxed);
        }
        fifo_index_.erase(oldest);
        ++n;
...
// Records restart-persistence tombstones for evictions that have become
// final (master notified, or no master to notify).  Must NOT be called
// for evictions that may still be rolled back.
void OffsetAllocatorStorageBackend::RecordEvictionTombstones(
    const PendingEviction& pending) {
    if (cfg_.persist_mode == OffsetPersistMode::kDisabled) return;
    for (const auto& [key, _] : pending.objects) {
        all_evicted_this_batch_.insert(key);
    }
    metadata_dirty_.store(true, std::memory_order_relaxed);
}
...
OffsetAllocatorStorageBackend::NotifyAndCommitPreparedEviction(
    ...
    if (!eviction_handler) {
        // No master to notify: dropping the handles commits the eviction
        // locally, so the keys still need tombstones to stay dead across
        // a restart.
        RecordEvictionTombstones(pending);
        pending.objects.clear();
        return {};
    }
    ...
    // The master accepted the removal: the eviction is now final, so
    // record the tombstones before dropping the handles.
    RecordEvictionTombstones(pending);
...
tl::expected<int64_t, ErrorCode> OffsetAllocatorStorageBackend::BatchOffload(
    ...
    for (const auto& [key, slices] : batch_object) {
        ...
        // Persistence tombstones for the victims are recorded inside
        // NotifyAndCommitPreparedEviction once the notification succeeds.
