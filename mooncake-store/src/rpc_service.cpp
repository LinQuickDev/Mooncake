...
tl::expected<bool, ErrorCode> WrappedMasterService::ExistKey(
    ...
    return execute_rpc(
        "ExistKey", PerfKey::MASTER_RPC_EXIST_KEY,
        [&] { return master_service_.ExistKey(key, tenant_id); },
...
tl::expected<void, ErrorCode> WrappedMasterService::Remove(
    ...
    return execute_rpc(
        "Remove", PerfKey::MASTER_RPC_REMOVE,
...
WrappedMasterService::RemoveHeartbeat(const UUID& client_id) {
    ScopedVLogTimer timer(1, "RemoveHeartbeat");
    timer.LogRequest("action=remove_heartbeat");
    return master_service_.RemoveHeartbeat(client_id);
}
...
void RegisterRpcService(
    ...
    server.register_handler<
        ...
        &wrapped_master_service);
    server.register_handler<
        &mooncake::WrappedMasterService::RemoveHeartbeat>(
        &wrapped_master_service);
    server
