...
class LocalSsdTaskMailbox {
   ...
   private:
    ...
    std::vector<RemoveTaskItem> pending_removes_ GUARDED_BY(mutex_);
...
class LocalSsdManager {
   public:
    ...
    ErrorCode EnqueueRemove(const UUID& client_id, const RemoveTaskItem& task);
    tl::expected<std::vector<RemoveTaskItem>, ErrorCode> PeekRemoves(
        const UUID& client_id) const;
    ErrorCode AckRemoves(const UUID& client_id,
