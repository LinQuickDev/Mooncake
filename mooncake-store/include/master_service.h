...
class MasterService {
   ...
   public:
    ...
    auto AckRemoveObjectHeartbeat(const UUID& client_id,
   ...
   private:
    ...
    struct ObjectMetadata {
        ...
        // Recovery protection must not overwrite a pending put/upsert action.
        // Reuse ordinary soft-pin index and metric bookkeeping.
        SoftPinEvaluation RefreshRecoveredSoftPin(uint64_t ttl_ms) {
            const auto now = std::chrono::system_clock::now();
            if (ttl_ms == 0) {
                return EvaluateSoftPin(now);
            }
            SpinLocker locker(&lock);
            const int metric_delta = soft_pin_timeout ? 0 : 1;
            soft_pin_timeout = ComputeSoftPinDeadline(now, ttl_ms);
            return {.active = true,
