...
TEST_F(BufferAllocatorTest, ImportedOffsetAllocationsLeaveAddressGapsFree) {
    constexpr uintptr_t kBase = 0x1A0000000ULL;
    constexpr size_t kCapacity = 4096;
    std::vector<LiveAllocation> allocations = {{64, 64}, {512, 64}};
    auto restored = ImportOffsetBufferAllocator(
        "restore-gaps", kBase, kCapacity, "restore-gaps", allocations);
    ASSERT_TRUE(restored.has_value());
    EXPECT_EQ(restored->allocator->size(), 128);
