...
TEST(LocalSsdManagerTest, RemovesAreTenantScopedAndRetainedUntilAcknowledged) {
    LocalSsdManager manager;
    const UUID client{5, 6};
    ASSERT_EQ(manager.RegisterClient(client, true), ErrorCode::OK);
    const RemoveTaskItem first{"tenant-a", "key"};
    const RemoveTaskItem second{"tenant-b", "key"};
    ASSERT_EQ(manager.EnqueueRemove(client, first), ErrorCode::OK);
    ASSERT_EQ(manager.EnqueueRemove(client, first), ErrorCode::OK);
    ASSERT_EQ(manager.EnqueueRemove(client, second), ErrorCode::OK);
    auto pending = manager.PeekRemoves(client);
    ASSERT_TRUE(pending.has_value());
    EXPECT_EQ(pending->size(), 2);
    EXPECT_EQ(manager.PeekRemoves(client)->size(), 2);
    ASSERT_EQ(manager.AckRemoves(client, {first}), ErrorCode::OK);
    pending = manager.PeekRemoves(client);
    ASSERT_TRUE(pending.has_value());
    ASSERT_EQ(pending->size(), 1);
    EXPECT_EQ(pending->front(), second);
    ASSERT_TRUE(manager.UnregisterClient(client).has_value());
    EXPECT_FALSE(manager.PeekRemoves(client).has_value());
    EXPECT_EQ(manager.AckRemoves(client, {second}),
