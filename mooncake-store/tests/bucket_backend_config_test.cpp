...
struct BucketBackendEnvironment {
    ScopedEnvVar disable_ssd_eviction{"MOONCAKE_OFFLOAD_DISABLE_SSD_EVICTION"};
    ScopedEnvVar gc_enable{"MOONCAKE_OFFLOAD_BUCKET_GC_ENABLE"};
    ScopedEnvVar gc_interval{"MOONCAKE_OFFLOAD_BUCKET_GC_INTERVAL_MS"};
    ScopedEnvVar gc_deleted_ratio{"MOONCAKE_OFFLOAD_BUCKET_GC_DELETED_RATIO"};
    ScopedEnvVar gc_watermark{
        "MOONCAKE_OFFLOAD_BUCKET_GC_HIGH_WATERMARK_RATIO"};
    ScopedEnvVar gc_max_buckets{
        "MOONCAKE_OFFLOAD_BUCKET_GC_MAX_BUCKETS_PER_ROUND"};
    ScopedEnvVar gc_merge{"MOONCAKE_OFFLOAD_BUCKET_GC_MERGE_ENABLE"};
...
TEST_F(BucketBackendConfigTest,
       PreservesSnapshotGcSettingsAlongsidePhysicalCap) {
    env.disable_ssd_eviction.Set("true");
    env.gc_enable.Set("false");
    env.gc_interval.Set("250");
    env.gc_deleted_ratio.Set("0.125");
    env.gc_watermark.Set("0.75");
    env.gc_max_buckets.Set("4");
    env.gc_merge.Set("false");
    env.max_physical_bytes.Set("2147483648");
    const auto config = BucketBackendConfig::FromEnvironment();
    EXPECT_TRUE(config.disable_ssd_eviction);
    EXPECT_FALSE(config.gc_enable);
    EXPECT_EQ(config.gc_interval_ms, 250);
    EXPECT_DOUBLE_EQ(config.gc_deleted_ratio, 0.125);
    EXPECT_DOUBLE_EQ(config.gc_high_watermark_ratio, 0.75);
    EXPECT_EQ(config.gc_max_buckets_per_round, 4);
    EXPECT_FALSE(config.gc_merge_enable);
