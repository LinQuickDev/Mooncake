...
class MasterServiceHATest : public ::testing::Test {
   protected:
    ...
    SoftPinDeadlineForTesting(MasterService& service, const std::string& key) {
        MasterService::MetadataAccessorRO accessor(
            &service, MasterService::ObjectIdentity{kDefaultTenant, key});
...
TEST_F(MasterServiceHATest, RemountMakesRestoredMemoryReplicaReady) {
    ...
    const auto lease_before =
        LeaseDeadlineForTesting(service, kDefaultTenant, first_key);
    EXPECT_EQ(MasterMetricManager::instance().get_allocated_mem_size() -
                  metric_before,
              2048);
...
TEST_F(MasterServiceHATest, FirstReadableRemountRefreshesSoftPinOnlyOnce) {
    MasterService service(MasterServiceConfig::builder()
                              .set_enable_ha(false)
                              .set_default_kv_lease_ttl(10000)
                              .set_default_kv_soft_pin_ttl(10000)
                              .build());
    const std::string key = "remount_soft_pin_key";
    const std::string endpoint = "remount_soft_pin_segment";
    auto object = MakeStandbyObject(key, endpoint);
    object.metadata.replicas.front()
        .get_memory_descriptor()
        .buffer_descriptor.buffer_address_ = kDefaultSegmentBase;
    ...
    const auto segment = MakeSegment(endpoint);
    ASSERT_TRUE(service.ReMountSegment({segment}, client_id).has_value());
    const auto deadline = SoftPinDeadlineForTesting(service, key);
    ASSERT_TRUE(deadline.has_value());
    EXPECT_GT(*deadline, std::chrono::system_clock::now());
    ASSERT_TRUE(service.ReMountSegment({segment}, client_id).has_value());
    EXPECT_EQ(SoftPinDeadlineForTesting(service, key), deadline);
}
