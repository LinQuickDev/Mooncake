...
控制 MC_LOG 系统的日志输出开关。仅影响使用 `MC_LOG`/`MC_VLOG` 的文件（real_client.cpp、client_service.cpp、transfer_task.cpp），不影响仍使用原生 `LOG()` 的文件。
...
- `store_py.cpp` 和 `urma_endpoint.cpp` 使用原生 glog，不受此开关控制
...
| 完全禁用日志输出 | `export MC_LOG_ENABLE=off` |
